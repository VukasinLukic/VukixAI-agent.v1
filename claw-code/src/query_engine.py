...
from .ollama_client import call_ollama, is_available
...
class QueryEnginePort:
    ...
    def submit_message(
        ...
        if is_available():
            ...
            if matched_commands:
                context_lines.append(f'Relevant commands: {", ".join(matched_commands)}')
            if matched_tools:
                context_lines.append(f'Relevant tools: {", ".join(matched_tools)}')
            output = call_ollama(prompt, context_lines=context_lines or None)
        else:
            summary_lines = [
                f'Prompt: {prompt}',
